...
//        else printf("[DELETE: First: -, Last: -] ");
    ...
    LList FreshCat(LList L1, LList L2){
        ...
        if (IsEmpty(L1) && IsEmpty(L2)) return L3;
        PNode node;

        node = L1->First;
        while (node != NULL){
            InsertEnd(node->Elem, L3);
            node = node->Next;
        }
    ...
    LList Cut(LList L, PNode P){
        if (!L || !P || IsEmpty(L)) exit(EXIT_SUCCESS);
        LList L3 = MakeEmpty(NULL);
    ...
    void InsertPos(TElem X, PNode P, LList L){
        ...
        PNode nodeP = L->First;
        while(nodeP != P) nodeP = nodeP->Next;

        PNode nodeX = malloc(sizeof(struct Node));
        PNode nodeS = nodeP->Next;
        nodeP->Next = nodeX;
        nodeX->Elem = X;
        nodeX->Next = nodeS;
    }